...
GPIO.setup(buzzerPin, GPIO.OUT)  # 부저 핀 설정
...
<body>
    <div class="container">
        ...
        <div class="led-control">
            <div>
                <p>Heat: {% if ledStates[0] == 1 %} ON {% else %} OFF {% endif %}</p>
                <a href="{{ url_for('LEDcontrol', LEDn=0, state=1) }}">
                    <button class="on" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>ON</button>
                </a>
                <a href="{{ url_for('LEDcontrol', LEDn=0, state=0) }}">
                    <button class="off" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>OFF</button>
                </a>
            </div>
            <div>
                <p>AC: {% if ledStates[1] == 1 %} ON {% else %} OFF {% endif %}</p>
                <a href="{{ url_for('LEDcontrol', LEDn=1, state=1) }}">
                    <button class="on" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>ON</button>
                </a>
                <a href="{{ url_for('LEDcontrol', LEDn=1, state=0) }}">
                    <button class="off" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>OFF</button>
                </a>
            </div>
            <div>
                <p>: {% if ledStates[2] == 1 %} ON {% else %} OFF {% endif %}</p>
                <a href="{{ url_for('LEDcontrol', LEDn=2, state=1) }}">
                    <button class="on" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>ON</button>
                </a>
                <a href="{{ url_for('LEDcontrol', LEDn=2, state=0) }}">
                    <button class="off" {% if auto_flag == 1 %} disabled class="disabled" {% endif %}>OFF</button>
                </a>
            </div>
        </div>
...
@app.route('/')
def index():
    distance = measure_distance()  # 초음파 센서로 측정한 거리 및 부저 제어
...
def LEDcontrol(LEDn, state):
    if auto_flag == 0:  # Manual 모드일 때만 LED 제어 가능
